// ===== map.js — Realistic map with Canvas graphics (no emojis) =====
...
class GameMap {
    ...
    _initMap() {
        ...
        for (let y=0;y<this.gridH;y++) { this.map[y]=[]; this.fogMap[y]=[]; for (let x=0;x<this.gridW;x++) { this.map[y][x]=TILE.EMPTY; this.fogMap[y][x]=true; } }
    }

    _resize() { const r=this.canvas.parentElement.getBoundingClientRect(); this.canvas.width=r.width; this.canvas.height=r.height; this.render(); }
    ...
    _s2g(sx,sy) { const r=this.canvas.getBoundingClientRect(); return {gx:Math.floor((sx-r.left-this.offsetX)/(this.cellSize*this.zoom)),gy:Math.floor((sy-r.top-this.offsetY)/(this.cellSize*this.zoom))}; }
    ...
    _paint(gx,gy) { if(gx<0||gx>=this.gridW||gy<0||gy>=this.gridH) return; this.map[gy][gx]=this.selectedTool; this.render(); if(this.onMapChange) this.onMapChange(this.map); }
    _movePlayer(id,gx,gy) { if(gx<0||gx>=this.gridW||gy<0||gy>=this.gridH) return; const t=this.map[gy][gx]; if(t===TILE.WALL||t===TILE.TREE||t===TILE.LAVA) return; this.players[id].x=gx; this.players[id].y=gy; this.render(); if(this.onPlayerMove) this.onPlayerMove(id,gx,gy); }

    setMapData(d) { if(!d) return; this.map=d; this.gridH=d.length; this.gridW=d[0]?d[0].length:this.gridH; this.render(); }
    ...
    _placePlayersAt(cx,cy) { let i=0; for(const[id,p] of Object.entries(this.players)) { p.x=Math.max(0,Math.min(this.gridW-1,cx+(i%3)-1)); p.y=Math.max(0,Math.min(this.gridH-1,cy+Math.floor(i/3))); i++; } }
    _placePlayersNearCenter() { const cx=Math.floor(this.gridW/2),cy=Math.floor(this.gridH/2); for(let r=0;r<15;r++) for(let dy=-r;dy<=r;dy++) for(let dx=-r;dx<=r;dx++) { const nx=cx+dx,ny=cy+dy; if(nx>=0&&nx<this.gridW&&ny>=0&&ny<this.gridH&&this.map[ny][nx]===TILE.FLOOR) { this._placePlayersAt(nx,ny); return; } } }

    // ===== MAP GENERATION =====
    generate(type) { this._initMap(); switch(type){case 'dungeon':this._genDungeon();break;case 'cave':this._genCave();break;case 'forest':this._genForest();break;case 'tavern':this._genTavern();break;case 'castle':this._genCastle();break;case 'temple':this._genTemple();break;case 'village':this._genVillage();break;case 'island':this._genIsland();break;default:this._genDungeon();} this.render(); if(this.onMapChange) this.onMapChange(this.map); }
    ...
    _genDungeon() {
        ...
        for(let i=1;i<rooms.length;i++) {
            ...
            const ax=a.x+Math.floor(a.w/2),ay=a.y+Math.floor(a.h/2),bx=b.x+Math.floor(b.w/2),by=b.y+Math.floor(b.h/2);
            let cx=ax,cy=ay;
    ...
    _genCave() {
        const W=this.gridW,H=this.gridH;
        for(let y=0;y<H;y++) for(let x=0;x<W;x++) this.map[y][x]=TILE.WALL;
        let grid=[]; for(let y=0;y<H;y++){grid[y]=[];for(let x=0;x<W;x++) grid[y][x]=Math.random()<0.45?1:0;}
        for(let iter=0;iter<5;iter++){const next=[];for(let y=0;y<H;y++){next[y]=[];for(let x=0;x<W;x++){let w=0;for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){const ny=y+dy,nx=x+dx;if(ny<0||ny>=H||nx<0||nx>=W)w++;else if(grid[ny][nx]===1)w++;}next[y][x]=w>=5?1:0;}}grid=next;}
    ...
    _genTavern() {
        ...
        this.map[hy+hh-1][hx+Math.floor(hw/2)]=TILE.DOOR;this.map[hy+hh-1][hx+Math.floor(hw/2)+1]=TILE.DOOR;
        ...
        this.map[hy+4][hx+hw-6]=TILE.DOOR;this.map[hy+1][hx+hw-2]=TILE.BED;this.map[hy+2][hx+hw-2]=TILE.CHEST;
    ...
    _genCastle() {
        ...
        this.map[oy+oh-1][Math.floor(ow/2)]=TILE.DOOR;this.map[oy+oh-1][Math.floor(ow/2)+1]=TILE.DOOR;
        ...
        this.map[ky+1][kx+Math.floor(kw/2)-1]=TILE.CHEST;this.map[ky+1][kx+Math.floor(kw/2)+1]=TILE.CHEST;
        for(const[tx,ty]of[[ox,oy],[ox+ow-3,oy],[ox,oy+oh-3],[ox+ow-3,oy+oh-3]]){for(let y=ty;y<ty+3;y++)for(let x=tx;x<tx+3;x++){if(y===ty||y===ty+2||x===tx||x===tx+2) this.map[y][x]=TILE.WALL;else this.map[y][x]=TILE.FLOOR;}this.map[ty+1][tx+1]=TILE.CHEST;}
    ...
    _genTemple() {
        ...
        this.map[cy-4][cx]=TILE.LAVA;this.map[cy-3][cx-1]=TILE.LAVA;this.map[cy-3][cx+1]=TILE.LAVA;
    ...
    _genVillage() {
        ...
        const houses=[];
        for(let i=0;i<12;i++){const hw=4+Math.floor(Math.random()*4),hh=3+Math.floor(Math.random()*3),hx=2+Math.floor(Math.random()*(W-hw-4)),hy=2+Math.floor(Math.random()*(H-hh-4));let ok=true;for(const h of houses){if(hx<h.x+h.w+2&&hx+hw>h.x-2&&hy<h.y+h.h+2&&hy+hh>h.y-2){ok=false;break;}}if(ok){houses.push({x:hx,y:hy,w:hw,h:hh});for(let y=hy;y<hy+hh;y++)for(let x=hx;x<hx+hw;x++){if(y===hy||y===hy+hh-1||x===hx||x===hx+hw-1) this.map[y][x]=TILE.WALL;else this.map[y][x]=TILE.FLOOR;}this.map[hy+hh-1][hx+Math.floor(hw/2)]=TILE.DOOR;this.map[hy+1][hx+1]=TILE.BED;if(Math.random()>0.5) this.map[hy+1][hx+hw-2]=TILE.CHEST;}}
    ...
    _genIsland() {
        ...
        for(let y=0;y<H;y++) for(let x=0;x<W;x++){const dist=Math.sqrt((x-cx)**2+(y-cy)**2);const noise=Math.sin(x*0.5)*Math.cos(y*0.5)*2+Math.sin(x*0.3+y*0.2)*3;if(dist<baseR+noise-2) this.map[y][x]=TILE.FLOOR;else if(dist<baseR+noise) this.map[y][x]=TILE.SAND;}
        ...
        this.map[cy-5][cx-5]=TILE.LAVA;this.map[cy-6][cx-5]=TILE.LAVA;this.map[cy-5][cx-4]=TILE.LAVA;
        this._placePlayersAt(cx,cy+5);
    }

    setMapFromAI(aiMapData) { if(!aiMapData||!aiMapData.length) return; const h=aiMapData.length,w=aiMapData[0]?aiMapData[0].length:0; if(h>this.gridH||w>this.gridW){this.gridH=Math.max(h,this.gridH);this.gridW=Math.max(w,this.gridW);} this._initMap(); for(let y=0;y<Math.min(h,this.gridH);y++) for(let x=0;x<Math.min(w,this.gridW);x++) this.map[y][x]=Math.max(0,Math.min(15,aiMapData[y][x]||0)); this._placePlayersNearCenter(); this.render(); if(this.onMapChange) this.onMapChange(this.map); }

    getMapDescription() {
        const features=[],positions=[];
        for(let y=0;y<this.gridH;y++) for(let x=0;x<this.gridW;x++){const t=this.map[y][x];if(t===TILE.DOOR)features.push(`дверь(${x},${y})`);else if(t===TILE.CHEST)features.push(`сундук(${x},${y})`);else if(t===TILE.TRAP)features.push(`ловушка(${x},${y})`);else if(t===TILE.STAIRS)features.push(`лестница(${x},${y})`);else if(t===TILE.THRONE)features.push(`трон(${x},${y})`);}
    ...
    render() {
        ...
        for(const[id,p]of Object.entries(this.players)) {
            ...
            grad.addColorStop(0,this._lighten(p.color,40)); grad.addColorStop(1,p.color);
            ...
            // Glow
            if(isMe) { ctx.shadowColor=p.color; ctx.shadowBlur=15; ctx.beginPath(); ctx.arc(px+cs/2,py+cs/2,r,0,Math.PI*2); ctx.stroke(); ctx.shadowBlur=0; }
    ...
    _drawTile(ctx, tile, px, py, cs) {
        ...
        switch(tile) {
            ...
            case TILE.WALL:
                ...
                ctx.lineWidth=1;
                const half=cs/2;
                ctx.strokeRect(px+m,py+m,cs-m*2,half-m);
                ctx.strokeRect(px+cs/4,py+half,cs/2,half-m);
                // Highlight
            ...
            case TILE.WATER:
                ...
                ctx.fillRect(px+cs*0.2,py+cs*0.1,cs*0.3,cs*0.15);
                break;
            ...
            case TILE.LAVA:
                ...
                ctx.fillRect(px+m,py+m,cs-m*2,cs-m*2);
            ...
            case TILE.TREE:
                ...
                ctx.fillRect(px+cs*0.4,py+cs*0.5,cs*0.2,cs*0.5);
                // Canopy
                ctx.fillStyle='#1e6e3e';
                ctx.beginPath(); ctx.arc(px+cs/2,py+cs*0.35,cs*0.35,0,Math.PI*2); ctx.fill();
                // Highlight
                ctx.fillStyle='rgba(100,255,100,0.15)';
                ctx.beginPath(); ctx.arc(px+cs*0.4,py+cs*0.3,cs*0.15,0,Math.PI*2); ctx.fill();
                break;
            ...
            case TILE.DOOR:
                ...
                ctx.beginPath(); ctx.arc(px+cs*0.7,py+cs/2,cs*0.06,0,Math.PI*2); ctx.fill();
                // Arch
                ctx.strokeStyle='#5d4037';
                ctx.lineWidth=2;
                ctx.beginPath(); ctx.arc(px+cs/2,py+m,cs/2-m,Math.PI,0); ctx.stroke();
                break;
            ...
            case TILE.CHEST:
                ...
                ctx.fillRect(px+cs*0.15,py+cs*0.35,cs*0.7,cs*0.5);
                // Lid
                ctx.fillStyle='#b7950b';
                ctx.fillRect(px+cs*0.15,py+cs*0.25,cs*0.7,cs*0.15);
                // Lock
                ctx.fillStyle='#f1c40f';
                ctx.fillRect(px+cs*0.42,py+cs*0.45,cs*0.16,cs*0.12);
                break;
            ...
            case TILE.STAIRS:
                ...
                for(let i=0;i<4;i++) { ctx.fillRect(px+cs*0.2,py+cs*(0.2+i*0.18),cs*0.6,cs*0.12); }
                break;
            ...
            case TILE.THRONE:
                ...
                // Throne
                ctx.fillStyle='#8b6914';
                ctx.fillRect(px+cs*0.2,py+cs*0.15,cs*0.6,cs*0.7);
                // Crown
                ctx.fillStyle='#f1c40f';
                ctx.beginPath();
                ctx.moveTo(px+cs*0.3,py+cs*0.3);
                ctx.lineTo(px+cs*0.5,py+cs*0.15);
                ctx.lineTo(px+cs*0.7,py+cs*0.3);
                ctx.fill();
                break;
            ...
            case TILE.BED:
                ...
                ctx.fillRect(px+cs*0.1,py+cs*0.2,cs*0.8,cs*0.65);
                // Pillow
                ctx.fillStyle='#ddd';
                ctx.fillRect(px+cs*0.15,py+cs*0.25,cs*0.3,cs*0.2);
                // Blanket
                ctx.fillStyle='#3498db';
                ctx.fillRect(px+cs*0.15,py+cs*0.5,cs*0.7,cs*0.3);
                break;
            ...
            case TILE.TABLE:
                ...
                // Table top
                ctx.fillStyle='#5d4037';
                ctx.fillRect(px+cs*0.1,py+cs*0.3,cs*0.8,cs*0.15);
                // Legs
